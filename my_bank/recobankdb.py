...
# mycon = sql.connect(host ='localhost', user ='root', passwd ='Oluwatobiloba07')
# mycursor = mycon.cursor()
# mycursor.execute("CREATE DATABASE recobank_db")
mycon = sql.connect(host ='localhost', user ='root', passwd ='Oluwatobiloba07', database = 'recobank_db')
mycursor = mycon.cursor()

# -- Create the Customers table to store customer information
# mycursor.execute("CREATE TABLE Customers(CustomerID VARCHAR(25) PRIMARY KEY, FirstName VARCHAR(25), LastName VARCHAR(25), DateOfBirth DATE, Email VARCHAR(25), Phone VARCHAR(20), Address VARCHAR(255))")


#  -- Create the Accounts table to store information about customer accounts
# mycursor.execute("CREATE TABLE Accounts (AccountID VARCHAR(25) PRIMARY KEY, CustomerID  VARCHAR(25), AccountType VARCHAR(50), AccountNumber INT(50) UNIQUE, Balance  DECIMAL(15, 2), FOREIGN KEY (CustomerID) REFERENCES Customers(CustomerID))")

# -- Create the Transactions table to track account transactions
# mycursor.execute("CREATE TABLE Transactions (TransactionID VARCHAR(50) PRIMARY KEY, AccountID VARCHAR(25), TransactionDate DATETIME, Amount DECIMAL(15, 2), TransactionType VARCHAR(50), FOREIGN KEY (AccountID) REFERENCES Accounts(AccountID))")

# -- Create a table for user authentication (simplified for demonstration)
# mycursor.execute("CREATE TABLE Users (UserID VARCHAR(25) PRIMARY KEY, Username VARCHAR(50) UNIQUE, PasswordHash VARCHAR(255), CustomerID VARCHAR(25), AccountNumber INT(50) UNIQUE, FOREIGN KEY (CustomerID) REFERENCES Customers(CustomerID))")
...
# mycon.commit()
# mycon.close()
# mycursor.execute("DROP TABLE Customers, Accounts, Transactions, Users")