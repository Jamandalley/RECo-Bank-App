...
class RECoBank:
    def __init__(self):
        self.conn = sql.connect(host ='localhost', user ='root', passwd ='Oluwatobiloba07', database = 'recobank_db')
    ...
    def create_customer(self):
        ...
        customer_id = str(uuid.uuid4()).split('-')
    ...
    #     pattern = r'^\w+@\w+\.\w+$'
    #     if re.match(self.email, pattern):
    ...
    def create_account(self):
        try:
            ...
            self.acct_num = random.randint(4100000000, 4199999999)
            self.customer_id = input("Enter your customer ID: ").strip()
            ...
            self.account_id = random.randint(10000000, 19999999)
            self.user_id = str(uuid.uuid4()).split('-')[-1]
    ...
    def transfer(self, customer_id):
        ...
        self.trans_id = str(uuid.uuid3()).split('-')[-1]
        try:
